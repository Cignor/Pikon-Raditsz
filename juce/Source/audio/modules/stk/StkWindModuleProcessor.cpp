...
StkWindModuleProcessor::StkWindModuleProcessor()
    : ModuleProcessor(
          BusesProperties()
              .withInput(
                  "Inputs",
                  juce::AudioChannelSet::discreteChannels(8),
                  true) // ch0: Freq Mod, ch1: Gate, ch2: Breath, ch3: Vibrato Depth, ch4: Vibrato
                        // Rate, ch5: Reed Stiffness, ch6: Jet Delay, ch7: Lip Tension
...
juce::AudioProcessorValueTreeState::ParameterLayout StkWindModuleProcessor::createParameterLayout()
{
    ...
    params.push_back(
        std::make_unique<juce::AudioParameterChoice>(
            ...
            "Instrument Type",
            juce::StringArray{"Flute", "Clarinet", "Saxophone", "Brass"},
            0));

    params.push_back(
        std::make_unique<juce::AudioParameterFloat>(
            paramIdBreathPressure,
            "Breath Pressure",
...
void StkWindModuleProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    juce::Logger::writeToLog(
        "[STK Wind] prepareToPlay: sampleRate=" + juce::String(sampleRate) +
    ...
    if (instrument)
    {
        ...
        juce::Logger::writeToLog(
            "[STK Wind] Instrument created and initialized at " + juce::String(sampleRate) + " Hz");
    }
#endif
...
void StkWindModuleProcessor::updateInstrument()
{
    ...
    {
        juce::Logger::writeToLog(
            "[STK Wind] EXCEPTION creating instrument: " + juce::String(e.what()));
        try
        {
            instrument = std::make_unique<stk::Flute>(20.0);
            if (instrument)
        ...
        {
            juce::Logger::writeToLog(
                "[STK Wind] EXCEPTION in fallback: " + juce::String(e2.what()));
        }
    }
#else
    ...
    juce::MidiBuffer&         midiMessages)
{
    ...
    const float baseBreathPressure =
        breathPressureParam != nullptr ? breathPressureParam->load() : 0.7f;
    ...
    for (int i = 0; i < buffer.getNumSamples(); ++i)
    {
        ...
        if (instrument)
        {
            ...
            instrument->controlChange(
                11, vibratoRate * 128.0f / 20.0f); // Vibrato Frequency (0-128 maps to 0-20 Hz)
...
bool StkWindModuleProcessor::getParamRouting(
    ...
    int&                outChannelIndexInBus) const
{
    ...
    if (paramId == paramIdBreathMod)
    {
        outChannelIndexInBus = 2;
        return true;
    }
    if (paramId == paramIdVibratoMod)
    {
        outChannelIndexInBus = 3;
        return true;
    }
    if (paramId == paramIdVibratoRateMod)
    {
        outChannelIndexInBus = 4;
        return true;
    }
    if (paramId == paramIdReedStiffnessMod)
    {
        outChannelIndexInBus = 5;
        return true;
    }
    if (paramId == paramIdJetDelayMod)
    {
        outChannelIndexInBus = 6;
        return true;
    }
    if (paramId == paramIdLipTensionMod)
    {
...
#if defined(PRESET_CREATOR_UI)
void StkWindModuleProcessor::drawParametersInNode(
    float                                                   itemWidth,
    ...
    const NodePinHelpers*                                   pinHelpers)
{
            ...
            ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
        ...
        if (breathPressure > 0.0f)
        {
            ...
            drawList->AddLine(
                ImVec2(p0.x, clampedBreathY), ImVec2(p1.x, clampedBreathY), breathColor, 1.5f);
        }
        ...
        const char* instrumentName = (currentInstType >= 0 && currentInstType < 4)
    ...
    if (breathMod)
        ImGui::BeginDisabled();
    float breath = breathPressureParam != nullptr
                       ? getLiveParamValueFor(
                             paramIdBreathMod, "breathPressure_live", breathPressureParam->load())
                       : 0.7f;
    if (ImGui::SliderFloat("##breath", &breath, 0.0f, 1.0f, "%.2f"))
    {
        if (!breathMod)
            if (auto* p = dynamic_cast<juce::AudioParameterFloat*>(
                    ap.getParameter(paramIdBreathPressure)))
                *p = breath;
    }
    if (ImGui::IsItemDeactivatedAfterEdit())
    {
    ...
    if (!breathMod)
        adjustParamOnWheel(ap.getParameter(paramIdBreathPressure), "breath", breath);
    if (breathMod)
        ImGui::EndDisabled();
    ...
    float vibratoRate =
        vibratoRateParam != nullptr
            ? getLiveParamValueFor(
                  paramIdVibratoRateMod, "vibratoRate_live", vibratoRateParam->load())
            : 5.0f;
    if (ImGui::SliderFloat("##vibratoRate", &vibratoRate, 0.0f, 20.0f, "%.1f Hz"))
    {
        if (!vibratoRateMod)
            if (auto* p =
                    dynamic_cast<juce::AudioParameterFloat*>(ap.getParameter(paramIdVibratoRate)))
                *p = vibratoRate;
    }
    if (ImGui::IsItemDeactivatedAfterEdit())
    {
    ...
    if (!vibratoRateMod)
        adjustParamOnWheel(ap.getParameter(paramIdVibratoRate), "vibratoRate", vibratoRate);
    if (vibratoRateMod)
        ImGui::EndDisabled();
    ...
    float vibratoDepth =
        vibratoDepthParam != nullptr
            ? getLiveParamValueFor(
                  paramIdVibratoMod, "vibratoDepth_live", vibratoDepthParam->load())
            : 0.2f;
    if (ImGui::SliderFloat("##vibratoDepth", &vibratoDepth, 0.0f, 1.0f, "%.2f"))
    {
        if (!vibratoMod)
            if (auto* p =
                    dynamic_cast<juce::AudioParameterFloat*>(ap.getParameter(paramIdVibratoDepth)))
                *p = vibratoDepth;
    }
    if (ImGui::IsItemDeactivatedAfterEdit())
    {
    ...
    if (!vibratoMod)
        adjustParamOnWheel(ap.getParameter(paramIdVibratoDepth), "vibratoDepth", vibratoDepth);
    if (vibratoMod)
        ImGui::EndDisabled();
    ...
    if (currentInstType == 1 || currentInstType == 2) // Clarinet or Saxophone
    {
        ...
        if (reedStiffnessMod)
            ImGui::BeginDisabled();
        float reedStiffness =
            reedStiffnessParam != nullptr
                ? getLiveParamValueFor(
                      paramIdReedStiffnessMod, "reedStiffness_live", reedStiffnessParam->load())
                : 0.5f;
        if (ImGui::SliderFloat("##reed", &reedStiffness, 0.0f, 1.0f, "%.2f"))
        {
            if (!reedStiffnessMod)
                if (auto* p = dynamic_cast<juce::AudioParameterFloat*>(
                        ap.getParameter(paramIdReedStiffness)))
                    *p = reedStiffness;
        }
        if (ImGui::IsItemDeactivatedAfterEdit())
        {
        ...
        if (!reedStiffnessMod)
            adjustParamOnWheel(
                ap.getParameter(paramIdReedStiffness), "reedStiffness", reedStiffness);
        if (reedStiffnessMod)
            ImGui::EndDisabled();
    ...
    else if (currentInstType == 0) // Flute
    {
        ...
        if (jetDelayMod)
            ImGui::BeginDisabled();
        float jetDelay =
            jetDelayParam != nullptr
                ? getLiveParamValueFor(paramIdJetDelayMod, "jetDelay_live", jetDelayParam->load())
                : 0.5f;
        if (ImGui::SliderFloat("##jet", &jetDelay, 0.0f, 1.0f, "%.2f"))
        {
            if (!jetDelayMod)
                if (auto* p =
                        dynamic_cast<juce::AudioParameterFloat*>(ap.getParameter(paramIdJetDelay)))
                    *p = jetDelay;
        }
        if (ImGui::IsItemDeactivatedAfterEdit())
        {
        ...
        if (!jetDelayMod)
            adjustParamOnWheel(ap.getParameter(paramIdJetDelay), "jetDelay", jetDelay);
        if (jetDelayMod)
            ImGui::EndDisabled();
    ...
    else if (currentInstType == 3) // Brass
    {
        ...
        if (lipTensionMod)
            ImGui::BeginDisabled();
        float lipTension =
            lipTensionParam != nullptr
                ? getLiveParamValueFor(
                      paramIdLipTensionMod, "lipTension_live", lipTensionParam->load())
                : 0.5f;
        if (ImGui::SliderFloat("##lip", &lipTension, 0.0f, 1.0f, "%.2f"))
        {
            if (!lipTensionMod)
                if (auto* p = dynamic_cast<juce::AudioParameterFloat*>(
                        ap.getParameter(paramIdLipTension)))
                    *p = lipTension;
        }
        if (ImGui::IsItemDeactivatedAfterEdit())
        {
        ...
        if (!lipTensionMod)
            adjustParamOnWheel(ap.getParameter(paramIdLipTension), "lipTension", lipTension);
        if (lipTensionMod)
            ImGui::EndDisabled();
